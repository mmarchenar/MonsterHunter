...
namespace MonsterHunter  // Define a namespace for the MonsterHunter project
{
    public class StrongState : IState  // Define the StrongState class that implements the IState interface
    {
        ...
        public StrongState(Hunter hunter, Map map)  // Constructor for StrongState that takes a Hunter object
        {
        ...
        public void ApplyState(Hunter hunter, Map map)  // Method to apply the effects of the Strong state to the hunter
        {
        ...
        public async Task expired(Hunter hunter, Map map)  // Asynchronous method to check if the state has expired
        {
            while (!isExpired)  // Loop until the state is marked as expired
            {
                if (HasExpired())  // Check if the Strong state has expired
                {
                    ...
                    map.info.Add($"Your Strength state expired");  // Notify that the Strong state has expired
